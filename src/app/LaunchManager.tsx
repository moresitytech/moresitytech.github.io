...
export function LaunchManager({ children }: { children: React.ReactNode }) {
          ...
          >
            ...
            <div className="max-w-3xl">
              ...
              <p className="mt-4 max-w-2xl text-pretty text-sm text-muted-foreground sm:text-base lg:text-lg">
                The website unlocks automatically at {launchInfo.label}. Until then, this screen
                keeps the launch moment front and center with a live countdown.
              </p>
            </div>
  ...
  return (
    <>
      {children}
      <AnimatePresence>{showReveal && <LaunchRevealOverlay />}</AnimatePresence>
    </>
  );
}
    ...
    >
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground sm:text-base">
          Thanks for waiting. The celebration blasts upward from the bottom before the full website
          opens.
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-2 text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground">
          <span className="rounded-full border border-border bg-surface-elevated px-3 py-1.5">
            Bottom-to-top blast
          </span>
          <span className="rounded-full border border-border bg-surface-elevated px-3 py-1.5">
            Celebration mode
